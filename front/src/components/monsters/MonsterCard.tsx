...
export function MonsterCard({monster, onUpgradeSkill, onDelete, isDeleting}: MonsterCardProps) {
    ...
    return (
        <div
            className={`relative flex flex-col h-full overflow-hidden rounded-2xl bg-zinc-900 border ${config.border} shadow-xl transition-all hover:shadow-2xl hover:-translate-y-1 hover:border-opacity-50 group`}>
                ...
                <div className="relative p-5 pb-0">
                    <div className="flex justify-between items-start">
                        ...
                        <div className="flex flex-col items-end">
                            ...
                            <span className="text-[10px] font-bold text-zinc-500 mt-1 uppercase">Niveau</span>
                        </div>
                    </div>
                ...
                <div className="p-5 grid grid-cols-2 gap-3">
                    <div className="bg-zinc-950/50 p-3 rounded-xl border border-white/5 flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <div className="text-[10px] font-bold text-zinc-500 uppercase">HP</div>
                            <div className="text-lg font-black text-white leading-none truncate">{monster.hp}</div>
                        </div>
                    </div>
                    <div className="bg-zinc-950/50 p-3 rounded-xl border border-white/5 flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <div className="text-[10px] font-bold text-zinc-500 uppercase">ATK</div>
                            <div className="text-lg font-black text-white leading-none truncate">{monster.atk}</div>
                        </div>
                    </div>
                    <div className="bg-zinc-950/50 p-3 rounded-xl border border-white/5 flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <div className="text-[10px] font-bold text-zinc-500 uppercase">DEF</div>
                            <div className="text-lg font-black text-white leading-none truncate">{monster.def}</div>
                        </div>
                    </div>
                    <div className="bg-zinc-950/50 p-3 rounded-xl border border-white/5 flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <div className="text-[10px] font-bold text-zinc-500 uppercase">VIT</div>
                            <div className="text-lg font-black text-white leading-none truncate">{monster.vit}</div>
                        </div>
                    </div>
                </div>
                        ...
                        {monster.skills.map((skill) => (
                            <div key={skill.num}
                                ...
                                <div className="flex justify-between items-start mb-2">
                                    <span className="text-sm font-bold text-white">Compétence #{skill.num}</span>
                                    <div
                                        className="text-[10px] font-bold text-zinc-500 bg-black/30 px-1.5 py-0.5 rounded">
                                        NIV {skill.lvl}/{skill.lvlMax}
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-3 text-xs text-zinc-400 mb-2">
                                    ...
                                    <span className="flex items-center gap-1"><Clock
                                        size={10}/> {skill.cooldown}s</span>
                                    <span
                                        className="text-purple-400 font-bold">{skill.ratio.percent}% {skill.ratio?.stat?.toUpperCase()}</span>
                                </div>
            ...
            {onDelete && (
                <div className="p-5 pt-0 mt-auto">
                    <button
                        onClick={onDelete}
                        disabled={isDeleting}
                        className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-red-500/10 text-red-500 font-bold text-sm hover:bg-red-500/20 border border-red-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed group/delete"
                    >
